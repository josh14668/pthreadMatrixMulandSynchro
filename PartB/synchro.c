...
int main(int argc, char ** argv)
{
	...
	pthread_t *thread = (pthread_t*)malloc((no_of_customers+1) * sizeof(pthread_t)); //total is No_Of_Customers + 1 to include farmer

	if(thread == NULL){
		fprintf(stderr, "threads out of memory\n");
		exit(1);
	}
	int * t_ids = malloc((no_of_customers+1) * sizeof(int)); //total is No_Of_Customers + 1 to include farmer
	if(t_ids == NULL){
...
////////////////////////////////////////////////////////////////////////
		...
		sleep((int)rand() % terminal_usage_time);
